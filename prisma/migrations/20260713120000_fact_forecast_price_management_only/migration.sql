-- FactForecast: resolve CPL from price_management_values only (no cpl_prices).
-- Prefer version-matched Fcst row, then Current Forecast fallback.
...
WITH fact_base AS (
  SELECT
    ...
    COALESCE(priceTarget.[cplPrice], priceCurrent.[cplPrice], 0) AS [cplPrice],
    COALESCE(
